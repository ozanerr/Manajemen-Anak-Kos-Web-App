					...
					<div className="hidden md:flex items-center space-x-1">
						{navItems.map(item => {
							...
							return (
								...
								>
									<Icon size={18} className="group-hover:rotate-12 transition-transform duration-200" />
					...
					<div className="md:hidden flex items-center">
						...
						>
							<div className="relative w-6 h-6">
								<Menu
									size={24}
									className={`absolute transition-all duration-300 ${open ? 'rotate-180 opacity-0' : 'rotate-0 opacity-100'}`}
								/>
								<X
									size={24}
									className={`absolute transition-all duration-300 ${open ? 'rotate-0 opacity-100' : '-rotate-180 opacity-0'}`}
								/>
							</div>
						</button>
					</div>
					...
					{navItems.map((item, index) => {
						...
						return (
							<Link
								key={item.name}
								to={item.path}
								className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-gray-700 hover:text-blue-600 hover:bg-white hover:shadow-md transition-all duration-200 transform hover:translate-x-2 ${open ? 'animate-slideIn' : ''
									}`}
								style={{
